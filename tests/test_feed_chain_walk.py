...
def test_unchained_tables_are_untouched():
    """Byte-identical behaviour with no chain: two independent tables, read separately, in
    `sorted(key=str)` order, with unqualified fragments."""
    g = _chained()
    g.remove((P1, TAB.continuesTable, P0))
    recs = table_records(g)
    assert len(recs) == 4
    assert [r.row_id for r in recs] == ["h0-r0 > h0-r0", "h0-r1 > h0-r1",
                                        "h0-r0 > h0-r0", "h0-r1 > h0-r1"]
