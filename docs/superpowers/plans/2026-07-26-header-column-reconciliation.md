...
> **SCOPE REVISED DURING EXECUTION (2026-07-26): shipped Cause A only.** Two corrections were made against the empirical suite: (1) the leaf rule became **label-center-in-column** (`colX0 ≤ inkCenterX < colX1`), not the originally-planned "column-center-in-ink" (which broke 26 left-aligned-label fixtures + needed a fallback that failed a short-parent case). (2) **Task 4 (caption peel) was implemented, then REVERTED** — no geometric signal separates a leaked caption from a genuinely-ambiguous off-center merge (both are overlapping top rows), so every peel broke the `test_offcenter_merge_escalates`/B1.3/span-gate escalation contracts. Caption handling is deferred to a NEURAL loop (see the spec §7). Tasks 1–3 + 5 (verify) shipped; Task 4 is void.
