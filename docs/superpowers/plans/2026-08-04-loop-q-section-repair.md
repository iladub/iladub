...
- [ ] **Step 1:** Build `multi_section_ruled_pdf` by refactoring `sectioned_ruled_table_pdf`'s body into a parameterized `_draw_section(c, y_off, key, notice, rows, truth)` helper both builders call (drawing identical: doubled outer border, strips, interior-ruled grid, leading header box with the wrapped `Time Nom`/`Accepted` name) plus, between sections, a total line (`sum of the Volume column, comma-formatted`) drawn full-width below each section's grid (`with_totals=True`). Keep `sectioned_ruled_table_pdf`'s public contract byte-identical (its tests must stay green — they are the loop-P pins).
...
- Driver order (spec §4.0, pinned by Task 1's monotonicity test): existing per-page compile → existing carriage → NEW: per page, `section_candidates` over STILL-ESCALATED ruled bands → pass-2 `compile_tables(page, section_repair_bands=group_indices, doc_uri=page_uri)` → adopt a band's pass-2 region IFF it asserts (regions list, graph triples for that table swapped in; the loop-M page-scoped URI idiom prevents collisions — mint pass-2 URIs under `{page_uri}/r2` and record adoption) → chains + totals.
