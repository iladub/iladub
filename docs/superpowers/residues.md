...
| R173 | open | Eight corpus-dependent tests fail locally and CI cannot see it — `corpus/` is gitignored, so they SKIP in CI and the `test` check is green on a tree where they fail; MEASURED byte-identical at the branch base and at HEAD, so the one-band merge causes none of them |
