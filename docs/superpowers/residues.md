...
| R152 | open | `sh:nodeKind sh:IRI` is VACUOUS at the compile membrane by construction — `_payload_nt` skolemizes every blank node into an IRI before a shape runs — while refusing correctly at `iladub.validate.validate`. Measured on the fixture written and then deleted while closing `R128`: same shapes, same data, `pyshacl_rdfs=False membrane=True`. `_payload_nt`'s own comment records the premise (*no shape file uses `sh:nodeKind`*) that makes skolemization verdict-neutral, and **nothing enforces it** |
