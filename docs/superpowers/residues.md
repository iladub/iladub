...
| R182 | open | A `tab:HeaderSourceCell` carries `sourceText` + `sourceRow` and no geometry, so no shape can check that a label's box COVERS the text it is the provenance of — the reason [[R181]]'s containment-shape arm was unexpressible rather than merely deferred |
