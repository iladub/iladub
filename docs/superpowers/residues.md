...
| R61 | closed | Emitter-typing is now an unenforced invariant — 0 UNTYPED since 2026-08-18; the live ones were emitter/ontology DISAGREEMENT **CLOSED 2026-09-08** (loop `the-axiom-binds-the-emitter`) — all 60 DISAGREE ruled (2 EMITTER, 5 AXIOM), **60 → 0** and live **32 → 0**, and the probe is WIRED as a CI gate on a synthetic graph. The row's own invited ruling for `atColumn` (`GridColumn ⊑ LeafColumn`) would have been a defect; and its *“nothing pins that carve-out”* has an answer — it cannot be pinned because the entry shape DOMINATES it. Gate reach is 5 of 7 rulings → [[R184]] |
...
| R160 | open | The datagrid adoption gate is pre-empted by one asserting band — apple p1's header band asserting means the page's own datagrid candidate is never offered adoption, `adopted (1,) → ()`, mechanism `adoption-candidate.rq`'s `NOT EXISTS tab:EntryCell`. A reader-authority question touching R73's monotonicity premise, not a bug **CENSUS 2026-09-05**: no corpus document adopts at all (7/7 `adopted=()`), though bfs p0/p4 still open the candidate gate and refuse at the re-compile — the branch, not the gate, is what has no live instance |
| R161 | closed | Loop Q's section repair never fires on apple p0/p2 — 8 of 11 escalations are `REGION_TILING_FAILED`, `DocumentReport.repaired_bands == ()` on both pages, cause unmeasured **CLOSED 2026-09-03 — measured with `scripts/section_repair_census.py`: recognition runs on every apple page (6/6/5 ruled bands), every band on p0 and p2 shares ONE rule-x signature, but the header-box half of `section-repeat.rq`'s identity is emitted for at most one band per page — and apple's sections print DIFFERENT headers, so the repair cannot fire by design. The eight refusals are one defect, `CoverageShape` on header-less section bands. Successor [[R165]]** |
| R162 | open | Unruled header labels are words — one `Line.words` abstraction carries two granularities (ruled cell vs pdfplumber word); grouping unruled multi-word spans into labels is NEURAL by CLAUDE.md §8, sibling of [[R155]] |
| R163 | open | The uncarried-ink guard's refusal carries no named reason — `MATRIX_AMBIGUOUS` now covers both "no anchor column" and "uncarried header ink" under one escalation string, `compile.py`'s single escalation site |
| R164 | open | Spec §1.5's corpus census for the matrix-body-start rule is a six-band oracle, four agreeing — LOW power, recorded so nobody reads it as confirmation |
| R165 | closed | **CLOSED 2026-09-04** (loop `the-run-is-one-band`) — `band-run.rq` proposes a contiguous run of ruled bands as ONE band and the existing tiling membrane disposes; 14 candidate runs on the corpus, 2 accepted (apple p0/p1, both page tails), apple document score **0.1895 → 0.6289**, IDENTICAL under `validate_shapes=True` — the membrane has now seen a merged band. Successors [[R170]] [[R171]] [[R172]]. Original: Apple's escalated statement bands are header-less intra-page CONTINUATIONS of one table (all 8 `REGION_TILING_FAILED` refusals are `CoverageShape`, every leaf column uncovered). **Its carriage remedy was REFUTED 2026-09-03**; the successor one-band prediction was RUN 2026-09-04 and is **CONFIRMED on p0/p1** (124 and 56 tiling entries vs 48 and 14 asserted today) and REFUSED on p2 ([[R167]] + [[R162]]). **Its own `tab:ruleXsSignature` licence was then RUN on the corpus 2026-09-04 and is REFUTED**: undershoots apple p1 (26 not 56 entries), misses apple p2's unruled header, and would swallow bfs p6's 216 asserted cells. The merge must be a PROPOSAL disposed by `classify_matrix`+`region_tiles`, not an unconditional split change. **Band-index renumbering MEASURED 2026-09-04: zero bands renumber on this corpus** (every accepted run is a page tail), apple document score **0.1895 → 0.6289** — but the index IS persisted into the shipped and grounded graphs and three two-pass flows depend on it. One design decision with [[R160]] |
| R166 | open | Two apple bands (p0 band 4, p2 band 6) assert a DATA ROW as their column header — `Operating income 35,695 …` as `h0..h4` of a `tab:RecordTable`; a §7 false assertion inside the membrane, one symptom of [[R165]] |
| R167 | open | The em-dash `—` (U+2014), US-GAAP's nil glyph, types as `tab:Text` — one such cell disqualifies a data column in `stub-data-split.rq` and moves apple p2's stub|data split from k=1 to k=2; `is_blank` covers the ASCII `-` but not `—`/`–` |
| R168 | open | `page_bands` is called at least twice per page with NO cache, so at least half of all band-building and disposal work is duplicated by construction — a perfect (pdf, page, srb) cache returns **41.54s** of the prototype's 83.77s, MORE than the 27.8s the one-band merge costs |
| R169 | open | Under M1 the merged band that SHIPS is not the merged band the oracle disposed of — the run is decided on the unrepaired build, the repair rebuild runs next, the splice last; on a page that both repairs and merges, `merge_bands` builds from repaired constituents no oracle saw |
| R170 | open | `is_matrix_candidate` is the SOLE guard on the asserted cells of every proposed run the derivation enumerates — forcing acceptance costs capacity p0 390→0, bfs p6 222→6, apple p2 3→0 (stem p0 UNCHANGED at 586, refuting the plan's own prediction); not guardable where the decision lives, so O3 is its standing detector |
| R171 | open | Two cases NO corpus document exercises — a non-tail ACCEPTED merge, and a page that both merges and ADOPTS; O5 forces both by patching the disposal, which proves the renumbering is consistent but not that any document needs it. Distinct from [[R169]] |
| R172 | closed | "124 entries vs 48 cells" compares `assert_matrix_region`'s return against `RegionReport.cells` — no content diff has ever been run, and O3 bounds the ink without identifying the cells **2026-09-05**: `test_an_adopted_page_never_scores_one_by_construction` is now explicitly this row's detector and is left FAILING by design; its old invariant is pinned separately on the synthetic adopting fixture | **CLOSED 2026-09-07** (loop `r172-the-cell-level-diff`) — the diff is RUN. apple p1 LOST 0 / CHANGED 0 / GAINED 42, every gain on a real baseline word, criterion met and `escalated=0` honest; apple p0 **REFUTES the row's own criterion in the merge's favour** — all 4 lost + 16 changed are band 4 (20 of 20), whose baseline `RECORD_TABLE` read a DATA ROW as its column header, so the criterion asked the merge to preserve a false assertion. No cell vanishes (each is an entry or a label), no gain invents ink — pinned by `test_the_merge_loses_no_cell_and_invents_no_ink`, falsified twice. The detector is DELETED with the ruling recorded, not re-baselined. Successor [[R176]] (the denominator moves: −24 words on p0, −28 on p1)
| R173 | open | Corpus-dependent tests fail locally and CI cannot see it — `corpus/` is gitignored, so they SKIP in CI and the `test` check is green on a tree where they fail. **BISECT RUN 2026-09-04: the eight are TWO findings** — the SEVEN apple failures begin at `4cfee38` ([[R160]]'s `adopted (1,) → ()`), the stem one at `20cc5b8` and is split out as [[R174]]. This row now covers the seven plus the CI-visibility half. Its own "byte-identical, the merge moves neither" claim is **counted right and set wrong**: the R165 merge repairs one of the eight and breaks another, and they cancel in the count **5a RUN 2026-09-05**: all 7 corpus documents adopt NOTHING, so the six adoption pins were re-pointed at a synthetic adopting fixture that already existed — not deleted, not weakened, each falsified — and the module went from 0 to 7 tests running in CI. The CI-visibility half stays OPEN and unpriced; one test is left RED on purpose ([[R172]]'s detector) |
| R174 | closed | `test_stem_document_is_byte_identical_under_adoption` pins `0.9654553611484971`; the tree has read `0.9658886894075404` since `20cc5b8` ([[R154]]). **RUN 2026-09-05 — CONFIRMED a pure denominator effect**: numerator byte-identical at 2152, denominator 2229→2228, one welded date-banner word that is not a cell; closure is now a mechanical re-baseline. That commit PUBLISHED the move in its own message and re-baselined three other pinned tests — all non-corpus **CLOSED 2026-09-05** — re-baselined to `0.9658886894075404` carrying [[R154]]'s ruling verbatim; `tests/test_corpus_stem.py` 13 passed |
| R175 | closed | The re-pointed adoption pins rest on a ONE-BAND synthetic fixture with no `ignored` region, so the control that keeps three of them non-vacuous stayed on apple and corpus-gated — five pins look CI-covered and their control does not run there **CLOSED 2026-09-06** — the band CAN be drawn: a `NON_TABLE` band books 0 escalated tokens and `compile.py:1276` supersedes only a report with `tokens_escalated > 0`, so it survives adoption as `ignored`. `currency_marker_escalating_with_note_pdf` (a sibling, one line of note) adopts at IDENTICAL page arithmetic, so no pin was re-baselined; the control now asserts both halves in CI and the module goes 7 → 8 CI tests |
| R176 | closed | The score's token denominator is **not stable across a verdict change** — a band that stops escalating and starts being read takes its row-stub ink OUT of `asserted+escalated` rather than moving it across, so two readings' scores are not comparable and `score=1.0` means *everything the denominator counts was read*. **CLOSED 2026-09-07** (loop `r176-the-denominator-that-moves`) — fork taken for (a), COUNT it; corpus-wide unbooked ink of asserted bands **262 words → 0**, 17 of 24 dirty bands → 0 of 24. The row's own prediction that apple p0/p1 stop reading 1.0 is **REFUTED**: their unbooked ink is 100% label-covered, so 1.0 was correct all along. Successors [[R177]], [[R178]] |
| R177 | closed | `assert_hier_region` emits every header `tab:LabelCell` **without** `tab:hasBBox`, unlike all five sibling emitters — 24 label cells on who-wfa p0 alone carry no provenance-to-the-page **CLOSED 2026-09-07** (loop `the-label-cell-that-cannot-be-found`) — geometry now carried through `HeaderNode` and written at the emitter; corpus bbox-less LabelCells **79 → 0**, all 27 page readings byte-identical. Promoted mid-loop from a provenance gap to [[R178]]'s **blocker**: it blinded `unbooked_ink_fate.py`, which reported **111** orphan words where only **12** are real. Successor [[R179]] (the membrane never required it) |
| R178 | closed | who-wfa's `Year: Month` axis caption is ink the matrix reader carries into NO structure — now booked `escalated` so it is visible, but still unread **CLOSED 2026-09-07** (loop `the-box-that-covers-one-line`) — ruled *measured, and not worth a reading change*, on a population corrected a THIRD time. The row's own "two questions" split is REFUTED: 8 of the 9 graincorp items are ink a label's `cellText` DOES carry, whose `hasBBox` covers only the last of three wrapped source lines — raised as [[R181]]. Genuinely unread: **4 word-groups in 2 of 7 documents**, all out-of-table captions, which is the shape the row predicted at raise time |
| R179 | closed | No SHACL shape requires `tab:hasBBox` on a `tab:LabelCell` — `tab:EntryCellPhysicalShape` requires it for entries only, so [[R177]]'s 79 boxless cells violated provenance-to-the-page with nothing validating it **CLOSED 2026-09-07** — `tab:LabelCellPhysicalShape` ships in the file AND in `tiling._PHYSICAL_SHAPE_IRIS` (the gate, not just final validation); corpus verdict diff WHOLE-FILE IDENTICAL on 27 pages. The row's own fork option (b) is REFUTED as unexpressible — no `tab:HeaderNode` carries geometry — so the condition is the label's own non-empty `tab:cellText`. Option (a) is raised as [[R180]] The corpus diff was NOT sufficient — it read IDENTICAL on a defective shape; the suite caught a full-closure divergence (`tab:hasLabel` range-types `rowgroups.py:93`'s EntryCell as a label), fixed by a provably-lossless `tab:EntryCell` exclusion |
| R180 | open | `span.build_reading`'s flank filler mints a `tab:LabelCell` with no text and no geometry — the single exemption `tab:LabelCellPhysicalShape` carves out. Whether it should be minted at all is an EMISSION question with `reshape.py:29`, the round-trip oracle, in its blast radius; zero corpus documents exercise the path |
| R181 | closed | A `tab:LabelCell` whose `cellText` is a wrap-join across N source lines carries a `hasBBox` covering only ONE of them — **7** of 17 labels on graincorp-stem p0 (the row said 8; 8 is the FRAGMENT count) **CLOSED 2026-09-07** (loop `the-box-that-covers-the-join`) — the site is `rowrole.py:174,182`, refuting BOTH of the row's suspects and its “sibling site, not the same one” caution; the fragments' bounds are now unioned into the label's box and graincorp-stem p0 orphan ink falls **9 → 1**. The fork's containment-shape arm is REFUTED as unexpressible — a `tab:HeaderSourceCell` carries no geometry — raised as [[R182]] |
| R182 | closed | A `tab:HeaderSourceCell` carries `sourceText` + `sourceRow` and no geometry, so no shape can check that a label's box COVERS the text it is the provenance of — the reason [[R181]]'s containment-shape arm was unexpressible rather than merely deferred **CLOSED 2026-09-08** (loop `the-box-must-cover-its-provenance`) — BOTH of the row's must-measure-first conditions were surprises: there is **no** label→source edge in either direction at any hop (worse), and `tab:hasBBox` on a `HeaderSourceCell` trips nothing **at the gate or the membrane** — but the loop then over-generalised that to *"the R19 mechanism is dead"*, emitted `tab:hasBBox`, and its OWN full suite refuted it (`test_closure_equiv.py`: 26 nodes refused by the full-RDFS leg the membrane is blind to). Geometry now rides dedicated `tab:sourceRegion`/`tab:sourcePage`, the `tab:markerRegion` precedent is VINDICATED, and [[R183]] is withdrawn. `tab:LabelCoversProvenanceShape` ships in the file AND in `tiling._PHYSICAL_SHAPE_IRIS`; corpus verdict diff WHOLE-FILE IDENTICAL on 27 pages; falsified by reverting [[R181]]'s union, under which the gate refuses the reading outright. the residue raised against `tab:markerRegion` ([[R183]]) is withdrawn in the same loop |
| R183 | closed | **WITHDRAWN, not repaired** — raised and retracted within [[R182]]'s own loop, before anything was built on it (the register has no `withdrawn` status; `tests/test_residue_register_integrity.py` routes on `open`/`closed` only, so it files as closed and says so in its first words). It claimed `tab:markerRegion` guards a dead hazard; the loop's full suite then REFUTED that — `membrane.rdfs_closure`'s owlrl leg still materialises `tab:hasBBox rdfs:domain tab:Cell` and refused 26 nodes the membrane admitted. The row is kept, struck, as the record of a wrong claim that was caught: **the membrane is not the only consumer of RDFS closure, and our PUBLISHED axioms bind consumers we do not run** |
| R184 | open | **The CI domain/range gate covers ONE emitter** — it is built from `emit_data_grid` alone, so it independently falsifies 5 of [[R61]]'s 7 rulings; `tab:hasLabel` (`rowgroups.py`) and every non-grid emitter are outside its CI reach, though the corpus probe still covers them |
| R185 | closed | **The release gate is hard-blocked and nothing can drain a contradiction** — the two 2026-08-10 decision-membrane docs still block, the spec's own “RESOLVED” note is invisible to `docgov_extract`'s regex, and `_since_date` excludes the tag being built so tagging does not self-clear. A DESIGN FORK (three arms), and the first task of any release loop **CLOSED 2026-09-08** — arm 2 (a drain register the AXIOM joins); gate exits 0 with both docs still declaring `contradiction`. The row's own refusal of arm 1 is WITHDRAWN and re-argued, and arm 3 is measured *vacuous*, not merely coarse. Successor [[R186]] |
| R186 | closed | **"Evidence is immutable after loop close" is a NORM, not a membrane** — nothing in `tests/test_doc_governance.py` checks it, and a spec did in fact gain an appended block after its loop closed with no instrument complaining. A DESIGN FORK now with **TWO** arms — arm 1 STRUCK 2026-09-08, the time separator cannot separate (73% vs 77% rewrite either side of the clock); both survivors are CLAUDE.md sentences and the MAINTAINER's to rule. Ruled the FIRST of two sequenced loops, before [[R187]] **CLOSED 2026-09-08** — the MAINTAINER ruled, taking neither arm as written but the proposal between them: *append-only, never rewritten*, `Doc impact:` frozen at VALUE granularity, binding prospectively from `5743af3`. Arm 3's premise (unenforceable-by-design) was itself falsified by the squash boundary. No lint ships — that is [[R188]] |
| R187 | closed | A wiki page states a corpus measurement in the **present tense** with no date on the claim — `data-grid.md` says *"stem unchanged at `0.9654553611484971`"* (×3) and apple `→ 0.35560344827586204`, and **both are superseded** ([[R174]] moved the stem to `0.9658886894075404`; apple reads `0.6289`); frontmatter `updated:` dates the file, not the claim. **RESCOPED 2026-09-08: the lint already EXISTS and already flags both pages** (warning-only, citation-gated, inline cites unextracted) — a recalibration of running code, and the SECOND of two sequenced loops |
| R188 | open | **Both doc-governance gates are calibrated to fire on ZERO** while the populations they guard sit in ungated zones beside them (0 vs 229 events; 0 vs 9 of 12 pages) — each exemption sized by an argument nobody re-checked. Generalised from n=2; **count before ruling, and it may be only two** |
| R189 | closed | **CLOSED 2026-09-09.** All four sites are now inside a HARD gate: the walk covers tracked `.py` prose (`test_no_source_file_states_an_undated_reading`), and all four — plus two the predecessor census had classified as false positives — were repaired by DATING the claim, never by rewriting a figure. Falsified: reverting one date turns the gate RED |
| R190 | open | **Nothing records which reading of a corpus quantity is CURRENT**, so no instrument can mark a figure superseded — only undated. MEASURED 2026-09-08: **4 of 7 corpus document scores moved in three days** and no tracked document records the new values. Do not close it with a hand-kept current-value column; the blocker is [[R173]] |
| R191 | open | `the-two-counts` §2's *"zero new stale quantities"* is REFUTED — CBH's `0.9047` moved twice unrecorded (`→0.9092→0.9095022624434389`). A METHOD row: a figure is only checkable against a reading taken today, so any future figure census runs the corpus first |
| R192 | open | **A handoff asserts CURRENT state, and `evidence` is the class every doc-governance gate exempts.** The capability line repeated by eight handoffs has **all four figures wrong**, and `bfs 0.9401` came from **C3, reverted at `51bdd23`** — a figure from a reverted branch. A design question with three asymmetric arms |
| R193 | open | **The figure gate's recall is a function of its own register**: `denotes` refuses any literal coarser than the register's derived separating precision (**4** today), which is what let the gate ship HARD with no tuned constant — and is also how it could go silently green. [[R188]]'s shape by a different route. Pinned by value; watch, do not fix |
| R194 | open | **`graincorp-capacity` is permanently outside the figure gate** (`cor:notQuotable`, 1 reading of 18): its score is exactly `1.0`, which is also how this repo writes a ratio ceiling. The exemption is what removed the last false positive and licensed the hard disposition. Close it by removing the NEED, never the mark |
| R195 | open | **Nothing enforces the one-PR-one-commit regime that the Evidence append-only rule is built on.** The ruleset's `pull_request` rule carries `allowed_merge_methods: ["merge","squash","rebase"]`; squash is a habit, not a constraint. A single merge-commit merge makes within-PR iteration indistinguishable from post-close editing — the exact conflation CLAUDE.md:617 chose the `5743af3` boundary to eliminate. Closure is a one-field ruleset edit, **owner-only** |
| R196 | open | **(a) is SETTLED — and its answer was wrong.** The candidate closure boundary (the comment run / docstring as the unit) **loses a real finding** — `test_datagrid.py:1085`, absolved by a date ten lines up about something else — and its one measured false positive is on inspection a finding. What the census confirmed is the prose *region*, not the boundary; the block stands. **(b) REMAINS, and this row's "both become live the moment the scope extends to code" is REFUTED for it**: measured at **0** over every `.py` prose block, so it is now dormant on two corpora, not one. Close (b) on the population an extension actually creates |
| R197 | open | **`.ttl` and `.rq` prose is outside the figure walk.** Extending it needs a Turtle/SPARQL lexer that tells a `#` comment from the `#` in an IRI and from one inside a string literal — Python hands that back for free in `tokenize`, Turtle does not. Measured population licensing the work: **1 undated occurrence across 203 files, a false positive, 0 findings** |
