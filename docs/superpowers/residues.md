...
| R165 | open | Apple's escalated statement bands are header-less intra-page CONTINUATIONS of one table (all 8 `REGION_TILING_FAILED` refusals are `CoverageShape`, every leaf column uncovered), and nothing carries a header from band to band within a page — loop M's carriage is page-to-page under `continuation-of.rq`, whose clause (a) can never license a header-less band. One design decision with [[R160]] |
