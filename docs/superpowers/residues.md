...
| R189 | open | The [[R187]] figure gate stops at tracked markdown — `src/iladub/etkl/compile.py:1288,1302` and `tests/etkl/test_datagrid.py:1085` state the superseded stem score `0.9654553611484971` as present fact, outside any gate. **CENSUS RUN 2026-09-09: scope was never the blocker** — all three sites write the reading at the end of a sentence, which the shipped `_DECIMAL` refused (fixed). Population is **4, not 3**; this row's own "mostly parameters" prior is REFUTED (0 of 13 false positives is a parameter); the real rate is 76%. Settle [[R196]] first |
...
| R196 | open | **The figure gate's `is_dated` has two scope defects, dormant only because the walk stops at markdown.** (a) In code the *block* is the wrong closure boundary — a docstring dated twice reports its first paragraph as undated (2 of 13 false positives in the [[R189]] census). (b) A date inside a filename dates the claim: `graincorp-stem-2026-07-31.pdf` silently dates 24 evidence occurrences with the document's date, not the reading's. **0 of 11 `docs/wiki/**` occurrences affected — nothing is lying today.** Do not fix before the scope moves |
