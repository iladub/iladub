...
| R177 | open | `assert_hier_region` emits every header `tab:LabelCell` **without** `tab:hasBBox`, unlike all five sibling emitters — 24 label cells on who-wfa p0 alone carry no provenance-to-the-page |
| R178 | open | who-wfa's `Year: Month` axis caption is ink the matrix reader carries into NO structure — now booked `escalated` so it is visible, but still unread; 3 words, one document, population unmeasured |
