...
| R181 | open | A `tab:LabelCell` whose `cellText` is a wrap-join across N source lines carries a `hasBBox` covering only ONE of them — 8 of 17 labels on graincorp-stem p0, the corpus's only such page. Provenance-to-the-page is PARTIAL where [[R177]] made it present; no shape enforces box-covers-text in either direction, and the joining site is NOT yet measured |
