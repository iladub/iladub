...
| R136 | open | A criterion can be `prog:met true` while its own `prog:source` points into *"Planned work (not done yet)"*, and NOTHING refuses it. M10 checks only that the `<path>:<line>` RESOLVES, never what the line says — measured by reverting this loop's own doc move and finding the suite fully green |
| R137 | open | The residue register has NO integrity check: deleting an index row outright, or flipping a closed row back to `open` while its detail row sits in `residues-closed.md`, both leave the suite GREEN. The only guard is M7, and it covers only the ~15 rows a `prog:blockedBy` names |
