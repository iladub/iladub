...
| R165 | open | Apple's escalated statement bands are header-less intra-page CONTINUATIONS of one table (all 8 `REGION_TILING_FAILED` refusals are `CoverageShape`, every leaf column uncovered). **Its carriage remedy was REFUTED 2026-09-03 by the forced-carriage spike**: apple mints no `CarriedHeaderReading` at all and the seam refuses a section-heading header row — the successor is a `page_bands` question (do not split at section headings), PROPOSED. One design decision with [[R160]] |
