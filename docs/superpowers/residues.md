...
| R179 (49/169 closed) | open | No SHACL shape requires `tab:hasBBox` on a `tab:LabelCell` — `tab:EntryCellPhysicalShape` requires it for entries only, so [[R177]]'s 79 boxless cells violated provenance-to-the-page with nothing validating it |
