...
| R176 | open | The score's token denominator is **not stable across a verdict change** — a band that stops escalating and starts being read takes its row-stub ink OUT of `asserted+escalated` rather than moving it across, so two readings' scores are not comparable and `score=1.0` means *everything the denominator counts was read*. MEASURED 2026-09-07 on apple p0 (`148→124`, −24 words) and p1 (`84→56`, −28) from the two accumulators themselves (`compile.py:744` books every word of an escalated band; `compile.py:923` books only an asserted matrix band's data cells). Structural, not introduced by [[R165]]. **A design fork — count label ink, or exclude it from both sides — must be taken before anything is written** |
