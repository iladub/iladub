...
| R143 | open | The declaration instrument's population is `.rq` ONLY — `vocab/shapes/*.ttl`, `examples/**` and `tests/*.ttl` name owned terms too, and are both unchecked and UNCENSUSED. This is the row that would subsume `R117`. **CENSUSED 2026-08-29: 209 undeclared across the four families, majority category error — the arc mints instance IRIs inside its own vocabulary namespace** |
