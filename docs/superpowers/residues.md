...
| R130 | open | `holon:05` §4.9's `(query, term)` vacuity tripwire was NOT built, so `membrane-health.rq`'s promotion clause stays a PROSE residue: over the 29 compile-path `.rq` queries the criterion yields **164** unreachable pairs, and **162 are a category error** (the query runs over a transient `urn:iladub:evidence:` graph, never the compiled one). Numbered R130, not the ruled R128 — see the row |
