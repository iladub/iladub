...
| R175 | open | The re-pointed adoption pins rest on a ONE-BAND synthetic fixture with no `ignored` region, so the control that keeps three of them non-vacuous stayed on apple and corpus-gated — five pins look CI-covered and their control does not run there |
