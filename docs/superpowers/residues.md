...
| R140 | open | The originating gate is configured never to enforce: plimslop ships `originating: block`, but `PLIMSLOP_MODE_ORIGINATING=warn` set inline on the Stop hook in machine-local `~/.claude/settings.json` downgrades it — re-measured 2026-08-26, **44/44 block records ran as `warn`**, and *not* via `stop.py:68`'s unsatisfiable-gate guard (0 of 44 had `baseline >= 50000`). Deferred in that order on purpose: restoring `block` under a floor unsatisfiable at turn one would make every session unstartable |
| R141 | open | The unit ruled 2026-08-26 is enforced nowhere: `stop.py:58` and `preflight` still gate on total tokens, and `reader.py` never reads `preflight` records — so the 54% override rate, **the ruling's own falsifying instrument**, is computed by no code. `measure.py:80` already derives `baseline` and every `turn` record carries it; nothing subtracts it. Blocked on one design call: a compaction lowers `tokens`, so naive subtraction falsely resets the floor |
