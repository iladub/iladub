...
| R154 | open | `rule_aware_lines` buckets per CHARACTER by centre, so a ruled boundary falling inside a word run shreds it — WHO's `Z-scores (weight in kg)` becomes 7 fragments, and chop/merge interleave so no downstream weld recovers it. Measured NOT to be the WHO `MATRIX_AMBIGUOUS` cause |
