...
| R186 | open | **"Evidence is immutable after loop close" is a NORM, not a membrane** — nothing in `tests/test_doc_governance.py` checks it, and a spec did in fact gain an appended block after its loop closed with no instrument complaining. A DESIGN FORK now with **TWO** arms — arm 1 STRUCK 2026-09-08, the time separator cannot separate (73% vs 77% rewrite either side of the clock); both survivors are CLAUDE.md sentences and the MAINTAINER's to rule. Ruled the FIRST of two sequenced loops, before [[R187]] |
