...
| R139 | open | A same-file citation pointing DOWNWARD is falsified by the very edit correcting it — the `holon:05` fix wave's first attempt shifted its own cited line numbers before the reflow was made line-neutral; "measure before writing" does not guard this class. **PARTIAL 2026-08-25: the convention half is now CLAUDE.md plan-rule 7; the row stays OPEN on its instrument half — nothing machine-checks it**. **SECOND measured instance 2026-08-31**, in `.md` this time, committed while applying the rule's own remedy — the instrument half now has two |
